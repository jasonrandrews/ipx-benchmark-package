...
#error "Please define a way to initialize a memory block."
  ...
  if (results[0].iterations == 0) {
    ...
    if (divisor == 0) /* some machines cast float to int as 0 since this conversion is not defined by ANSI, but we know
...
#if HAS_FLOAT
    if (known_id == 3) {
      ee_printf("CoreMark 1.0 : %f / %s %s", default_num_contexts * results[0].iterations / time_in_secs(total_time),
